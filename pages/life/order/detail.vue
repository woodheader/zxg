<template>
  <view v-if="!isLoading" class="container">
    ...
    <!-- 服务列表 -->
    <view class="life-list i-card">
      <view class="life-item" v-for="(life, idx) in order.life" :key="idx">
        <view class="life-main" @click="handleTargetLife(life.life_id)">
          <!-- 服务图片 -->
        ...
        <!-- 服务售后 -->
        <!--view class="life-refund">
          <text v-if="life.refund" class="stata-text">
		  <view style="text-decoration: underline;" v-if="life.refund.refuse_desc" @click="showRejectReason(life.refund.refuse_desc)">{{life.refund.state_text}}</view>
		  <view v-else>{{life.refund.state_text}}</view>
		  </text>
          <view v-else-if="order.isAllowRefund" class="action-btn" @click.stop="handleApplyRefund(life.order_life_id)">申请售后</view>
        </view-->
      </view>
    </view>
    ...
    <view v-if="order.order_status != OrderStatusEnum.CANCELLED.value" class="footer-fixed">
      <view class="btn-wrapper">
        ...
        <block v-if="order.delivery_status == DeliveryStatusEnum.DELIVERED.value && order.receipt_status == ReceiptStatusEnum.NOT_RECEIVED.value">
          <view class="btn-item active" @click="onReceipt(order.order_id)">服务满意，结单</view>
        </block>
        <!-- 订单评价 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 跳转到服务详情页面
      handleTargetLife(lifeId) {
        this.$navTo('pages/life/detail', { lifeId })
      },

      // 跳转到申请售后页面
      handleApplyRefund(orderLifeId) {
        this.$navTo('pages/refund/apply', { orderLifeId })
      },
