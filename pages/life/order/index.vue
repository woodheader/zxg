        ...
        <view class="order-item" v-for="(item, index) in list.data" :key="index">
          ...
          <view class="order-total">
            <!--view style="font-size: 16rpx;">
              <text>预约服务时间：{{ item.serve_time }}</text>
            </view-->
            <text>共{{ item.total_num }}件服务，总金额</text>
            <text class="unit">￥</text>
          ...
          <view class="btn-item-tihuo" style="background-color: #fff;">
            ...
            <text style="color:#E6C48D" v-else>已支付-></text>
            <text style="color:#777" v-if="item.is_jiedan == JieDanEnum.NO.value">待接单-></text>
            <text style="color:#E6C48D" v-else>已接单-></text>
            <text style="color:#777" v-if="item.receipt_status == ReceiptStatusEnum.NOT_RECEIVED.value">待完成</text>
            <text style="color:#E6C48D" v-else>已完成</text>
          </view>
				</block>
				<block v-if="item.order_status != OrderStatusEnum.CANCELLED.value">
					<!-- 未支付取消订单 -->
					...
					<!-- 已申请取消 -->
					<view v-else class="f-28 col-8 btn-item-cacel">取消申请中...</view>
					<!-- 未支付的订单 -->
					<block v-if="item.pay_status == PayStatusEnum.PENDING.value">
					...
					<block v-if="item.delivery_status == DeliveryStatusEnum.DELIVERED.value && item.receipt_status == ReceiptStatusEnum.NOT_RECEIVED.value">
					  <view class="btn-item active" @click="onReceipt(item.order_id, item.points_bonus)">确认验收</view>
					</block>
					<!-- 订单评价 -->
			  ...
			  <!-- 已取消或已完成订单可删除 -->
			  <block v-if="item.order_status == OrderStatusEnum.CANCELLED.value || item.order_status == OrderStatusEnum.COMPLETED.value">
				  <view class="btn-item" @click="onRemove(item.order_id)">删除</view>
			  </block>
            </view>
          </view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 结单
      onReceipt(orderId, pointsBonus) {
        const app = this
        OrderApi.receipt(orderId)
          .then(result => {
          uni.showModal({
            title: '订单完成',
            content: '系统已赠送知珠 ' + pointsBonus + ' 个',
            cancelText: '返回首页',
            confirmText: '查看账户',
            success(o) {
              if (o.confirm) {
