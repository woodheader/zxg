<template>
  <!-- 服务评价 -->
  <view v-if="!isLoading && list.length" class="life-comment m-top20">
    <view class="item-title dis-flex">
      <view class="block-left flex-box">
        服务评价 (<text class="total">{{ total }}条</text>)
      </view>
  ...
  export default {
    ...
    methods: {
      ...
      getCommentList() {
        const app = this
        app.isLoading = true
        CommentApi.listRows(app.lifeId, app.limit)
          .then(result => {
      ...
      onTargetToComment() {
        const app = this
        app.$navTo('pages/life/comment/index', { lifeId: app.lifeId })
      }
