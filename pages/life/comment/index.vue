  ...
  <mescroll-body ref="mescrollRef" :sticky="true" @init="mescrollInit" :down="{ use: false }" :up="upOption" @up="upCallback">
    ...
    <view class="comment-list">
      <view class="comment-item" v-for="(item, index) in list.data" :key="index">
        ...
        <!-- 商品规格 -->
        <view class="life-props clearfix" v-if="item.orderLifeRela">
          <view class="life-props-item" v-for="(props, idx) in item.orderLifeRela.life_props" :key="idx">
...
<script>
  ...
  import * as CommentApi from '@/api/life/comment'
    ...
    methods: {
      ...
      getCommentList(pageNo = 1) {
        ...
        return new Promise((resolve, reject) => {
          CommentApi.list(app.lifeId, { scoreType: app.getScoreType(), page: pageNo }, { load: false })
            .then(result => {
              // 合并新数据
