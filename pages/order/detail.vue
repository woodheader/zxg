    ...
    <view class="goods-list i-card">
      <view class="goods-item" v-for="(goods, idx) in order.goods" :key="idx">
        ...
        <!--view class="goods-refund">
          <text v-if="goods.refund" class="stata-text">
		  <view style="text-decoration: underline;" v-if="goods.refund.refuse_desc" @click="showRejectReason(goods.refund.refuse_desc)">{{goods.refund.state_text}}</view>
		  <view v-else>{{goods.refund.state_text}}</view>
		  </text>
