        ...
        <view class="order-item" v-for="(item, index) in list.data" :key="index">
          ...
          <view class="order-handle">
            <view class="btn-group clearfix">
				<!-- 订单提货方式 -->
				<block class="tihuo">
				  <view class="btn-item-tihuo" v-if="DeliveryTypeEnum.EXPRESS.value == item.delivery_type">{{DeliveryTypeEnum.EXPRESS.name}}订单</view>
				  <view class="btn-item-tihuo" v-if="DeliveryTypeEnum.ZITI.value == item.delivery_type">{{DeliveryTypeEnum.ZITI.name}}订单</view>
				</block>
				<block v-if="item.order_status != OrderStatusEnum.CANCELLED.value">
					...
					<block v-if="item.delivery_status == DeliveryStatusEnum.DELIVERED.value && item.receipt_status == ReceiptStatusEnum.NOT_RECEIVED.value">
					  <view class="btn-item active" @click="onReceipt(item.order_id, item.points_bonus)">确认收货</view>
					</block>
					<!-- 订单评价 -->
